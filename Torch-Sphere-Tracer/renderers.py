...
class SphereTracing(torch.autograd.Function):
    ...
    def forward(
        ...
        *parameters,
        ...
        with torch.no_grad():
            c_new = torch.rand(1,1,3, dtype = torch.float , device = "cuda:0")
            for i in range(num_iterations):
                signed_distances, c  = signed_distance_function(ray_positions)
                if i:
                    ray_positions = torch.where(foreground_masks & ~converged, ray_positions + ray_directions * signed_distances, ray_positions)
                    color = torch.where(foreground_masks & ~converged, torch.zeros_like(c) , c )
                else:
        ...
        return ray_positions, converged ,color
    ...
    def backward(ctx, grad_outputs, _):
        ...
        with torch.enable_grad():
            ...
            signed_distances, color  = signed_distance_function(ray_positions)
...
def compute_shadows(
    ...
    surface_positions, converged, color = sphere_tracing(
...
def compute_normal(
    ...
    if finite_difference_epsilon:
        ...
        surface_normals_x, _ = signed_distance_function(surface_positions + finite_difference_epsilon_x) - signed_distance_function(surface_positions - finite_difference_epsilon_x)
        surface_normals_y,_ = signed_distance_function(surface_positions + finite_difference_epsilon_y) - signed_distance_function(surface_positions - finite_difference_epsilon_y)
        surface_normals_z,_ = signed_distance_function(surface_positions + finite_difference_epsilon_z) - signed_distance_function(surface_positions - finite_difference_epsilon_z)
        ...
        with torch.enable_grad():
            signed_distances,_ = signed_distance_function(surface_positions)
